...
RUN pip install --no-cache-dir --user \
    "llmops-common @ git+https://github.com/${GIT_ORG}/llmops-common.git@${GIT_REF}#subdirectory=llmops-common"
...
RUN set -eu; \
    ...
    for lab in ${INCLUDE_LABS}; do \
      case "$lab" in \
        rag) url="rag-benchmark-lab @ git+https://github.com/${GIT_ORG}/rag-benchmark-lab.git@${GIT_REF}#subdirectory=rag-benchmark-lab" ;; \
        promptops) url="promptops-lab @ git+https://github.com/${GIT_ORG}/promptops-lab.git@${GIT_REF}#subdirectory=promptops-lab" ;; \
        schema) url="schema-lab @ git+https://github.com/${GIT_ORG}/schema-lab.git@${GIT_REF}#subdirectory=schema-lab" ;; \
        review) url="review-lab @ git+https://github.com/${GIT_ORG}/review-lab.git@${GIT_REF}#subdirectory=review-lab" ;; \
        memory) url="memory-lab @ git+https://github.com/${GIT_ORG}/memory-lab.git@${GIT_REF}#subdirectory=memory-lab" ;; \
        "") continue ;; \
...
RUN if [ "${INCLUDE_STUDIO}" = "true" ]; then \
      pip install --no-cache-dir --user --no-deps \
        "llmops-studio @ git+https://github.com/${GIT_ORG}/llmops-studio.git@${GIT_REF}#subdirectory=llmops-studio" && \
